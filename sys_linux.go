...
func keyctl_SetTimeout(id keyId, nsecs uint) error {
	_, _, errno := syscall.Syscall(syscall.SYS_KEYCTL, uintptr(keyctlSetTimeout), uintptr(id), uintptr(nsecs))
	if errno != 0 {
		return errno
	}
	return nil
...
func keyctl_Read(id keyId, b *byte, size int) (int32, error) {
	v1, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlRead), uintptr(id), uintptr(unsafe.Pointer(b)), uintptr(size), 0, 0)
	if errno != 0 {
		return -1, errno
	}
...
func keyctl_Link(id, ring keyId) error {
	_, _, errno := syscall.Syscall(syscall.SYS_KEYCTL, uintptr(keyctlLink), uintptr(id), uintptr(ring))
	if errno != 0 {
		return errno
	}
	return nil
...
func keyctl_Unlink(id, ring keyId) error {
	_, _, errno := syscall.Syscall(syscall.SYS_KEYCTL, uintptr(keyctlUnlink), uintptr(id), uintptr(ring))
	if errno != 0 {
		return errno
	}
	return nil
...
func keyctl_Chown(id keyId, user, group int) error {
	_, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlChown), uintptr(id), uintptr(user), uintptr(group), 0, 0)
	if errno != 0 {
		return errno
	}
	return nil
...
func keyctl_SetPerm(id keyId, perm uint32) error {
	_, _, errno := syscall.Syscall(syscall.SYS_KEYCTL, uintptr(keyctlSetPerm), uintptr(id), uintptr(perm))
	if errno != 0 {
		return errno
	}
	return nil
...
func add_key(keyType, keyDesc string, payload []byte, id int32) (int32, error) {
	...
	r1, _, errno = syscall.Syscall6(syscall.SYS_ADD_KEY,
...
func getfsgid() (int32, error) {
	...
	a1 = -1
	if r1, _, errno = syscall.Syscall(syscall.SYS_SETFSGID, uintptr(a1), 0, 0); errno != 0 {
		err = errno
		return int32(-1), err
	}
	return int32(r1), nil
...
func newKeyring(id keyId) (*keyring, error) {
	r1, _, errno := syscall.Syscall(syscall.SYS_KEYCTL, uintptr(keyctlGetKeyringId), uintptr(id), uintptr(1))
	if errno != 0 {
		return nil, errno
	}
...
func searchKeyring(id keyId, name, keyType string) (keyId, error) {
	...
	r1, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlSearch), uintptr(id), uintptr(unsafe.Pointer(b1)), uintptr(unsafe.Pointer(b2)), 0, 0)
	if errno != 0 {
		err = errno
	}
	return keyId(r1), err
...
func describeKeyId(id keyId) ([]byte, error) {
	...
	sizeRead = size + 1
	for sizeRead > size {
		r1, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlDescribe), uintptr(id), uintptr(unsafe.Pointer(&b1[0])), uintptr(size), 0, 0)
		if errno != 0 {
			return nil, errno
		}
...
func listKeys(id keyId) ([]keyId, error) {
	...
	sizeRead = size + 1
	for sizeRead > size {
		r1, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlRead), uintptr(id), uintptr(unsafe.Pointer(&b1[0])), uintptr(size), 0, 0)
		if errno != 0 {
			return nil, errno
		}
...
func updateKey(id keyId, payload []byte) error {
	...
	_, _, errno := syscall.Syscall6(syscall.SYS_KEYCTL, uintptr(keyctlUpdate), uintptr(id), uintptr(unsafe.Pointer(&payload[0])), uintptr(size), 0, 0)
	if errno != 0 {
		return errno
	}
	return nil
